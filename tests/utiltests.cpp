...
TEST_F(UtilTest, filename_hint) {
  std::string path;
  std::string cmdline = "C:\\Program Files (x86)\\MSBuild\\14.0\\bin\\amd64\\MSBuild.exe";
  bool status = etw::ExtractCmdlinePath(cmdline, path, "MSBuild.exe");
  EXPECT_FALSE(status);
  EXPECT_EQ(cmdline, path);
}

TEST_F(UtilTest, filename_hint2) {
  std::string path;
  std::string cmdline = "\\??\\C:\\Program Files (x86)\\MSBuild\\14.0\\bin\\amd64\\MSBuild.exe some params here";
  std::string expected = "C:\\Program Files (x86)\\MSBuild\\14.0\\bin\\amd64\\MSBuild.exe";
  bool status = etw::ExtractCmdlinePath(cmdline, path, "MSBuild.exe");
  EXPECT_FALSE(status);
